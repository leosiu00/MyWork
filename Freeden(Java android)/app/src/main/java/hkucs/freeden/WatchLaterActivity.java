...
public class WatchLaterActivity extends AppCompatActivity {
    ...
    private final static String TITLE = "陣間睇";
    private boolean deletion_mode = false;

    private DBMaster db;
    Toolbar toolbar;
    SharedPreferences pref;
    private static final String TAG = WatchLaterActivity.class.getSimpleName();
    PopUpPreview popup;
    private Menu menu;
...
//                Toast.makeText(view.getContext(), String.format("https://lihkg.com/thread/%d/page/1", p.getId(), Toast.LENGTH_SHORT).show();
                if(deletion_mode) {
                    adapter.removeWatchLaterRecordFromWatchLaterSet(p.getId());
                    adapter.getView(i, view, adapterView);
                    db.unwatchLaterPostThread(p.getId());
                    adapter.notifyDataSetChanged();
                } else {
...
//                Toast.makeText(view.getContext(), String.format("https://lihkg.com/api_v1_1/thread/%d/page/1", ((PostThread) adapterView.getItemAtPosition(i)).getId()), Toast.LENGTH_SHORT).show();
                ...
                popup = new PopUpPreview(WatchLaterActivity.this, p, adapter.isWatchLaterRecord(i), adapter.isFavouriteRecord(i), adapter.isBlockedUser(i));
                ...
                new AlertDialog.Builder(this)
                        ...
                        .setPositiveButton("是", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialogInterface, int i) {
                                adapter.updateWatchLaterSet();
                                db.unwatchLaterAllPostThreads();
                                runOnUiThread(new Runnable() {
                                    @Override
                                    public void run() {
                                        adapter = new PostThreadListAdapter(WatchLaterActivity.this, post_thread_list);
...
//            update_count = 0;
    ...
    public void FetchPosts() {
        Integer[] post_id_arr = db.getAllWatchLaterPostThreadIdsInArray();
                    ...
                    try {
                        ...
                        if( reader.getInt("success")==0 ) {
                            ...
                            runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    Toast.makeText(WatchLaterActivity.this
