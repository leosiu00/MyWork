...
public class HistoryActivity extends AppCompatActivity {
    ...
    private final static String TITLE = "回　帶";
...
//                Toast.makeText(view.getContext(), String.format("https://lihkg.com/thread/%d/page/1", p.getId(), Toast.LENGTH_SHORT).show();
                if(deletion_mode) {
                    adapter.removeHistoryRecordFromHistorySet(p.getId());
                    adapter.getView(i, view, adapterView);
                    db.deleteHistoryPostThread(p.getId());
                    adapter.notifyDataSetChanged();
                } else {
...
//                Toast.makeText(view.getContext(), String.format("https://lihkg.com/api_v1_1/thread/%d/page/1", ((PostThread) adapterView.getItemAtPosition(i)).getId()), Toast.LENGTH_SHORT).show();
                ...
                popup = new PopUpPreview(HistoryActivity.this, p, adapter.isWatchLaterRecord(i), adapter.isFavouriteRecord(i), adapter.isBlockedUser(i));
                ...
                new AlertDialog.Builder(this)
                        ...
                        .setPositiveButton("是", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialogInterface, int i) {
                                adapter.updateHistorySet();
                                db.deleteAllHistoryPostThread();
                                runOnUiThread(new Runnable() {
                                    @Override
                                    public void run() {
                                        adapter = new PostThreadListAdapter(HistoryActivity.this, post_thread_list);
